...
PSQL="psql --username=sebosuar --dbname=worldcup --no-align --tuples-only -c"
...
echo $($PSQL "SELECT name FROM teams FULL JOIN games ON teams.team_id = games.game_id WHERE year=2018 AND round='Final' AND winner_goals > opponent_goals")
